document.addEventListener('DOMContentLoaded', async () => {
    ...
    `}).join('');
        ...
        window.reimprimirRomaneio = async (romaneioId) => {
            ...
            // v3.11.34 — Detecta dados corrompidos e abre modal de recuperação antes de imprimir
            const _isBad = (v) => !v || v === 'undefined' || v === 'null' || String(v).trim() === '' || String(v).trim() === 'NÃO INFORMADO';
            const corruptedItems = fullItems.filter(item => _isBad(item.client) || _isBad(item.city));

            if (corruptedItems.length > 0) {
                console.warn(`[Reimprimir v3.11.34] ${corruptedItems.length} NF(s) com dados corrompidos. Abrindo modal de recuperação.`);
                window._showRomaneioRecoveryModal(manifest, fullItems, (correctedItems) => {
                    if (window.printSpecificRomaneio) window.printSpecificRomaneio(manifest.carrier, correctedItems);
                });
                return;
            }
        ...
        // v3.11.34 — Modal de recuperação de dados corrompidos no romaneio
        window._showRomaneioRecoveryModal = (manifest, items, onConfirm) => {
            const existing = document.getElementById('romaneioRecoveryModal');
            if (existing) existing.remove();

            const _isBad = (v) => !v || v === 'undefined' || v === 'null' || String(v).trim() === '' || String(v).trim() === 'NÃO INFORMADO';

            // Copia profunda dos itens para edição
            let editableItems = items.map(item => ({ ...item }));

            // Tenta auto-recuperar via snapshot do próprio manifest
            editableItems.forEach(item => {
                const snap = (manifest.items || []).find(s => s.id === item.id || s.invoice === item.invoice);
                if (snap) {
                    if (_isBad(item.client)  && !_isBad(snap.client))  item.client  = snap.client;
                    if (_isBad(item.city)    && !_isBad(snap.city))    item.city    = snap.city;
                    if ((!item.nfValue || item.nfValue === 0) && snap.nfValue > 0) item.nfValue = snap.nfValue;
                    if ((!item.total   || item.total   === 0) && snap.total   > 0) item.total   = snap.total;
                }
            });

            const stillCorrupted = editableItems.filter(i => _isBad(i.client) || _isBad(i.city));
            const romDate = manifest.createdAt ? new Date(manifest.createdAt).toLocaleDateString('pt-BR') : '?';

            const modal = document.createElement('div');
            modal.id = 'romaneioRecoveryModal';
            modal.style.cssText = 'position:fixed;inset:0;z-index:99999;background:rgba(0,0,0,0.75);backdrop-filter:blur(4px);display:flex;align-items:center;justify-content:center;padding:1rem;';

            modal.innerHTML = `
                <div style="background:var(--surface-color,#1e1e2e);border:1px solid rgba(245,158,11,0.4);border-radius:12px;width:100%;max-width:900px;max-height:90vh;overflow-y:auto;padding:1.5rem;box-shadow:0 25px 60px rgba(0,0,0,0.5);">
                    <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:1rem;">
                        <span class="material-icons-round" style="color:#f59e0b;font-size:1.8rem;">warning</span>
                        <div>
                            <h3 style="margin:0;color:#f59e0b;font-size:1.1rem;">Dados Corrompidos — Recuperação Manual</h3>
                            <p style="margin:0;font-size:0.8rem;color:var(--text-secondary,#888);">
                                Romaneio <strong>${manifest.id}</strong> · ${manifest.carrier} · ${romDate} · ${editableItems.length} NFs
                            </p>
                        </div>
                    </div>
                    <p style="font-size:0.82rem;color:var(--text-secondary,#888);margin-bottom:1rem;line-height:1.5;">
                        ⚠️ <strong>${stillCorrupted.length}</strong> nota(s) com dados ausentes.
                        Preencha os campos destacados antes de imprimir. Campos com ✅ foram recuperados automaticamente.
                    </p>
                    <div style="overflow-x:auto;">
                        <table style="width:100%;border-collapse:collapse;font-size:0.8rem;">
                            <thead>
                                <tr style="background:rgba(255,255,255,0.05);">
                                    <th style="padding:6px 8px;text-align:left;border-bottom:1px solid rgba(255,255,255,0.1);white-space:nowrap;">Nº NF</th>
                                    <th style="padding:6px 8px;text-align:left;border-bottom:1px solid rgba(255,255,255,0.1);min-width:180px;">Cliente</th>
                                    <th style="padding:6px 8px;text-align:left;border-bottom:1px solid rgba(255,255,255,0.1);min-width:120px;">Cidade</th>
                                    <th style="padding:6px 8px;text-align:right;border-bottom:1px solid rgba(255,255,255,0.1);white-space:nowrap;">Vlr NF (R$)</th>
                                    <th style="padding:6px 8px;text-align:right;border-bottom:1px solid rgba(255,255,255,0.1);">Frete (R$)</th>
                                </tr>
                            </thead>
                            <tbody>
                                ${editableItems.map((item, idx) => {
                                    const badClient = _isBad(item.client);
                                    const badCity   = _isBad(item.city);
                                    const rowBg     = (badClient || badCity) ? 'rgba(245,158,11,0.08)' : 'transparent';
                                    const badge     = (badClient || badCity) ? '' : '<span style="color:#22c55e;font-size:0.7rem;">✅</span>';
                                    const safeClient = (!badClient) ? String(item.client).replace(/"/g,'&quot;') : '';
                                    const safeCity   = (!badCity)   ? String(item.city).replace(/"/g,'&quot;')   : '';
                                    return `<tr style="background:${rowBg};border-bottom:1px solid rgba(255,255,255,0.05);">
                                        <td style="padding:4px 8px;font-weight:600;white-space:nowrap;">${badge} ${item.invoice || 'S/N'}</td>
                                        <td style="padding:4px 6px;">
                                            <input type="text" id="rec_client_${idx}" value="${safeClient}" placeholder="${badClient ? 'Digite o cliente...' : safeClient}"
                                                style="width:100%;background:${badClient ? 'rgba(245,158,11,0.1)' : 'rgba(255,255,255,0.05)'};border:1px solid ${badClient ? 'rgba(245,158,11,0.6)' : 'rgba(255,255,255,0.1)'};border-radius:4px;padding:4px 6px;color:inherit;font-size:0.8rem;">
                                        </td>
                                        <td style="padding:4px 6px;">
                                            <input type="text" id="rec_city_${idx}" value="${safeCity}" placeholder="${badCity ? 'Digite a cidade...' : safeCity}"
                                                style="width:100%;background:${badCity ? 'rgba(245,158,11,0.1)' : 'rgba(255,255,255,0.05)'};border:1px solid ${badCity ? 'rgba(245,158,11,0.6)' : 'rgba(255,255,255,0.1)'};border-radius:4px;padding:4px 6px;color:inherit;font-size:0.8rem;">
                                        </td>
                                        <td style="padding:4px 6px;text-align:right;">
                                            <input type="number" id="rec_nfval_${idx}" value="${parseFloat(item.nfValue)||0}" step="0.01" min="0"
                                                style="width:85px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:4px;padding:4px 6px;color:inherit;font-size:0.8rem;text-align:right;">
                                        </td>
                                        <td style="padding:4px 6px;text-align:right;">
                                            <input type="number" id="rec_total_${idx}" value="${parseFloat(item.total)||0}" step="0.01" min="0"
                                                style="width:85px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:4px;padding:4px 6px;color:inherit;font-size:0.8rem;text-align:right;">
                                        </td>
                                    </tr>`;
                                }).join('')}
                    ...
                    <div style="display:flex;gap:0.75rem;justify-content:flex-end;margin-top:1.25rem;flex-wrap:wrap;">
                        <button id="recoveryBtnCancel" class="btn btn-secondary" style="padding:0.5rem 1.25rem;">Cancelar</button>
                        <button id="recoveryBtnPrint" class="btn btn-primary" style="padding:0.5rem 1.5rem;display:flex;align-items:center;gap:0.5rem;">
                            <span class="material-icons-round" style="font-size:1.1rem;">print</span>
                            Imprimir com estes dados
                        </button>
                    </div>
                </div>`;
            ...
            document.getElementById('recoveryBtnCancel').onclick = () => modal.remove();
            modal.addEventListener('click', (e) => { if (e.target === modal) modal.remove(); });

            document.getElementById('recoveryBtnPrint').onclick = () => {
                const correctedItems = editableItems.map((item, idx) => ({
                    ...item,
                    client:  (document.getElementById(`rec_client_${idx}`)?.value || '').trim() || item.client || 'NÃO INFORMADO',
                    city:    (document.getElementById(`rec_city_${idx}`)?.value   || '').trim() || item.city   || 'NÃO INFORMADO',
                    nfValue: parseFloat(document.getElementById(`rec_nfval_${idx}`)?.value) || 0,
                    total:   parseFloat(document.getElementById(`rec_total_${idx}`)?.value) || 0,
                }));
                const stillBad = correctedItems.filter(i => _isBad(i.client) || _isBad(i.city));
                if (stillBad.length > 0) {
                    const nfs = stillBad.map(i => i.invoice || 'S/N').join(', ');
                    if (!confirm(`⚠️ ${stillBad.length} nota(s) ainda sem cliente/cidade: ${nfs}.\n\nImprimir mesmo assim?`)) return;
                }
                modal.remove();
