...
// ═══════════════════════════════════════════════════════════════════════════════
//  ErpNFQueue — Fila Universal de NFs do ERP (Maxdata / Acontec / Multi-Tenant)
// ═══════════════════════════════════════════════════════════════════════════════
window.ErpNFQueue = {
    _sales: [],
    ...
    async init() {
        ...
        try {
            ...
            console.warn('[ErpNFQueue] Erro no init:', e);
        }
    },
    ...
    async refresh() {
        ...
        try {
            ...
            if (tbody && (!this._sales || this._sales.length === 0)) {
                tbody.innerHTML = `<tr><td colspan="10" style="text-align:center; padding:1.2rem; color:#f87171; font-size:0.8rem;">Erro ao conectar com o ERP: ${e.message}</td></tr>`;
            }
        } finally {
    ...
    discardSale(saleId) {
        ...
        this.render(this._sales);
        if (typeof showToast === 'function') showToast(`🗑️ NF #${nfNum} descartada da fila.`, 'info');
    ...
    async selectSale(saleId) {
        ...
        if (!sale) {
            if (typeof showToast === 'function') showToast('❌ Dados da NF não encontrados.', 'error');
            return;
        }
    ...
    fillAndLockQuote(sale) {
        ...
        if (sale.cpfCnpj || sale.clienteId || sale.clienteNome) {
            const allClients = (typeof Utils !== 'undefined' && Utils.getStorage('clients')) || [];
        ...
        if (typeof showToast === 'function') showToast(`📥 Dados do ERP carregados (${refText})!`, 'success');
    ...
    onConfirmed(invoice) {
        if (!invoice) return;
        this._addDiscardedId(invoice);
        this._sales = this._sales.filter(s => String(s.numeroNf) !== String(invoice) && String(s.id) !== String(invoice));
        this.render(this._sales);
    },
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    try {
        ...
        window.printInvoiceReport = (historyId) => {
            ...
            // Abre a janela imediatamente para o navegador não bloquear o popup
            const printWindow = window.open('', '_blank', 'width=900,height=700');
            printWindow.document.write('<html><head><meta charset="UTF-8"></head><body style="display:flex;align-items:center;justify-content:center;height:80vh;margin:0;"><div style="font-family:sans-serif;font-size:1.1rem;color:#64748b;display:flex;align-items:center;gap:10px;"><span>⏳ Carregando relatório de conferência...</span></div></body></html>');
            ...
            // Executa a busca assíncrona em background
            (async () => {
                let dispatches = [];
                ...
                const relatedNFs = dispatches.filter(d => {
                    if (entry.nfList && entry.nfList.length > 0) {
                        return entry.nfList.includes(String(d.invoice));
                    }
        ...
        // ───────────────────────────────────────────────────────────────────────────
        // DASHBOARD LOGIC — Etapa 4: Movido para dispatch-dashboard.js (v3.12.5)
        // Funções: renderDashboard, openShipmentModal, toggleNFSelection,
        //          undoDispatch, generateRomaneioAction
        // O arquivo dispatch-dashboard.js é carregado antes de app.js (ver index.html)
        // ───────────────────────────────────────────────────────────────────────────

        // Render inicial do dashboard (dispatch-dashboard.js também tem DOMContentLoaded,
        // mas esta chamada garante render após o login completar o loadAll)
    ...
    `}).join('');
        ...
        // ── FILA DE NFs DO ERP & BLOQUEIO INTELIGENTE (ErpNFQueue unificado no topo do script) ───────────
        ...
        window._buildVendorWAMsg = function (d, dispatchDate) {
            ...
            const _redespLine = _redespC ? `\n📦 Redespacho (entrega final): ${_redespC}` : '';

            switch (tenant) {
                case 'altafix':
                    return `Olá ${sellerName}!\nInformamos que o pedido do cliente ${client}, com nº de NF: ${invoice}, com ${volume} volumes, no valor de ${nfValue}, foi despachado em ${date} via ${carrier}.${_redespLine}\nPrevisão de entrega: ${leadTime} dias.\nAlta Fix Distribuidora de Peças agradece!\nQualquer dúvida, estamos à disposição.`;
