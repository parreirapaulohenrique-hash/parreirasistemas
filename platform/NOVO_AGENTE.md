...
|---|---|---|
| **3.21.37** | 2026-09-17 | Integrações ERP / Despacho: Resolução robusta de tenant no `ErpUI` e `ErpRegistry` via URL pathname (`/centralpecas`), query string, `parreira_session` e fallbacks. Elimina erro `tenantId é obrigatório para salvar config` e impede que a tela de integração pareça desconfigurada ao recarregar a página. |
| **3.21.36** | 2026-09-17 | WMS Coletor (v3.18.10): Suporte provisório para NFs homologadas 15105, 15107 e 35845 (concluídas na Tela 102 do ERP MaxData) com carga de SKUs e códigos de barras para conferência física no coletor, com filtro de descarte automático após recebimento. |
...
| **3.21.29** | 2026-09-16 | Segurança / Multi-Tenant (Isolamento e Precedência Absoluta): Correção de conflito de sessão entre tenants no Despacho Logístico quando acessado por URL direta (precedência absoluta do tenant da URL sobre sessões gravadas em storage, descartando sessões divergentes no SSO). Restrição do Painel Admin (Master) exclusivamente ao tenant 'parreira'. Bloqueio funcional e visual de módulos não contratados no Hub e validação de permissões no Bússola Gestão (ERP Consultoria) e MAXCRM. |
| **3.21.27** | 2026-09-16 | Despacho Logístico (v3.21.27): Correção de transportadoras de redespacho (TRANSPORTE TUERE) na Conferência de Fatura. Inclusão no seletor independente de valor pré-calculado, alocação de taxas por volume/TDA ao fornecedor de redespacho no motor de cálculo de frete, conciliação balanceada da fatura da transportadora principal (desconto do redespacho) e reparo retroativo de lançamentos no Firestore. |
| **3.21.25** | 2026-09-16 | Arquitetura / Autenticação (Modelo A): Implantação do Login Unificado por Tenant com Matriz de Permissões por Módulo. Validação de perfil no ParreiraAuth (`users_index` + `tenants/{id}/users`), matriz dinâmica de módulos no Hub da plataforma, SSO transparente no Despacho Logístico (bypassa modal de login se já autenticado) e suporte a autenticação rápida por PIN de 4 dígitos em terminais de bancada de expedição. |
| **3.21.24** | 2026-09-15 | Hub / Login: Correção do loop de redirecionamento para o módulo despacho (tenant centralpecas). Removido o redirecionamento forçado de tenants não-parreira para /:tenant no index.html; o hub da plataforma agora permanece acessível a todos os tenants autenticados. Limpeza de parreira_session_ls nos logouts e suporte a logout explícito no login.html sem BOM UTF-8. |
...
| **3.21.51** | 2026-09-22 | FIX: Relatório de Impressão de Faturas — correção da coluna 'Frete Base' quando a conferência for de transportadora de redespacho (ex: CAMINHAO BELEM). O relatório trazia indevidamente o frete base da transportadora principal; agora calcula e exibe o frete base e taxas da própria transportadora de redespacho, garantindo conformidade com a tabela e a coluna Total. |
